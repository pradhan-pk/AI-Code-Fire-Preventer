...
app = FastAPI(title="AI Code Impact Analyzer API")
...
@api_router.get("/")
async def root():
    return {"message": "AI Code Impact Analyzer API", "status": "running"}
...
async def create_project(project: ProjectCreate):
    ...
    result = db.create_project(project_id, project.name, project.description)
    return result
...
async def create_repository(repo: RepositoryCreate, background_tasks: BackgroundTasks):
    ...
    result = db.create_repository(repo_data)
    
    # Start processing in background
    background_tasks.add_task(repo_processor.process_repository, repo_id)
    
    return result

@api_router.get("/projects/{project_id}/repositories")
async def get_repositories(project_id: str):
    return db.get_repositories(project_id)

@api_router.get("/projects/{project_id}/modules")
async def get_project_modules(project_id: str):
    return db.get_project_modules(project_id)

@api_router.get("/projects/{project_id}/analyses")
async def get_recent_analyses(project_id: str, limit: int = 10):
    return db.get_recent_analyses(project_id, limit)

# GitHub webhook endpoint
@api_router.post("/webhook/github/{repo_id}")
async def github_webhook(repo_id: str, request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        background_tasks.add_task(
            analysis_engine.analyze_commit,
            repo_id,
            commit_hash,
            list(changed_files),
        ...
        return {"message": "Webhook processed", "commit": commit_hash}
